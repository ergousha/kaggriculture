...
# v0.4.x ships the multi-route public chassis: main.py has no
# `_ROUTE_B85_PARTS` and is regenerated by scripts/build_v041.py (byte-verified
# against the committed v0.4.0 snapshot), not by the route-replay template.
# These tests are the template-pipeline contract for route-replay-era agents.
ROUTE_REPLAY_ERA = "_ROUTE_B85_PARTS" in open(MAIN).read()

pytestmark = pytest.mark.skipif(
    not ROUTE_REPLAY_ERA,
    reason="main.py is the chassis-era multi-route agent (v0.4.x); the "
    "route-replay template pipeline is retired (see scripts/build_v041.py)",
)
