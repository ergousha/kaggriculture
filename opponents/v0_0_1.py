...
# Tunable constants.
#
# Tuning status is stated for every value. "TUNED" means the local arena's
# paired-seed A/B (`local_arena.py --ablate`) measured it; "PLACEHOLDER" means it
# is a reasoned starting value that has NOT yet been arena-validated.
# ---------------------------------------------------------------------------
...
#   PRICE_FLOOR_SELLING -- holding capped goods above a price floor measured
#     -1.8% (p~0.00014, better on only 6/30 seeds) in self-play, the case it was
#     designed for. Melon price never recovers enough for the held stock to sell,
#     and unsold stock scores $0. Selling everything immediately is simply better.
#   PREDATORY_TIMING -- fired only once that floor bound, so with the floor gone
#     it was unreachable. Measured exactly 0 delta over 80 paired seeds.
#   OPPONENT_ADAPTIVE -- backing melon land off when the opponent farms melon too
#     measured -26.9% vs adaptive (p~0.0, better on 0/30 seeds) and -26.4% in
#     self-play. The melon pot goes to whoever harvests into it first, so racing
#     for it beats conceding it; the marginal price matters less than the tempo.

# Labour. Hire cost is fib(n) for the n-th hire of the day: 1,1,2,3,5,8,13,21...
...
# BUILD outranks PLANT: a coop starts compounding on day 4 and a built coop is
# the precondition for spending cash on a goose, whereas a melon seed pays
# nothing for 12 days. With PLANT first, melon planting crowded out coops.
PRIO_BUILD = 560
PRIO_PLANT = 550
...
class StrategicPlanner:
    ...
    def plan_roles(self, farm, day, days_left, market, cash):
        ...
        if not FLAGS["EXPAND_LAND"]:
            melon_target = min(melon_target, max(0, len(usable) // 4))
        ...
        roles = {}
        idx = 0
        # Premium livestock first (highest $/action while their pot is unspoiled).
        ...
        # Melon is capped BOTH by the size of its market pot (MELON_TILE_TARGET)
        # and by a share of current land. The share matters: early on we hold
        # only the 24-tile NW quadrant, and melon pays nothing for 12 days while
        # a coop compounds from day 4. Allocating melon out of the residual
        # starved the engine to a single coop, so it is explicitly second in line.
        melon_n = min(melon_target, int(remaining * MELON_LAND_FRACTION))
        rest = max(0, remaining - melon_n)

        # Coops are built JUST IN TIME against cash. A coop is only worth a tile
        # once we can also buy and feed the goose that lives in it: planning 20
        # coops on day 2 with $3,000 produced 20 empty coops, a starved flock and
        # no cash crop, which is a death spiral. So the coop count tracks the
        # birds we already own plus what we can afford right now.
        ...
        # Wheat is sized to the flock we actually expect to feed, not to the
        # leftover land.
        wheat_n = min(rest - coop_n, max(1, int(math.ceil(coop_n * WHEAT_TILES_PER_ANIMAL))))
        # Anything still spare goes to WHEAT, not a cash crop. Spare-land wheat
        # displaces purchased feed at ~$50/unit (the buy side ramps on sqrt), and
        # any true surplus still sells at ~$19-25 because wheat's glut curve is
        # log. Carrot was measured at ~$6/action here and it craters its own
        # price: 660 carrots over a season drives it to the $1 floor.
        wheat_n += max(0, rest - coop_n - wheat_n)
        ...
        for _ in range(wheat_n):
            if idx < len(usable):
                ...
                idx += 1
        # Melons go on the outer ring: one visit/day, so travel matters least.
    ...
    def market_orders(self, obs, farm, private, market, opp, day, days_left, hour, log):
        ...
        for animal, cap, min_days in (
            ...
            # Cap the un-placed backlog. Ferrying is rate-limited to
            # FERRY_MAX_UNITS chains per turn, so buying ahead of that just
            # parks cash in the shed: unbounded, this sank every dollar into 40
            # idle geese and left nothing for seeds or feed.
            want = min(want, max(0, ANIMAL_BACKLOG_CAP - in_stock))
            if want <= 0:
                continue
            cost = ANIMALS[animal]["cost"]
            # Feed gate: FEED consumes one wheat per animal per day and two
            # consecutive unfed days makes the animal escape for good. Buying
            # ahead of the feed supply destroyed the stock outright (measured:
            # geese placed on day 1 with an empty shed were gone by day 2).
            ...
            if feed_on_hand < feed_needed:
                continue
            budget = cash if animal == "GOOSE" else discretionary
        ...
        for crop, last_day in (("MELON", MELON_LAST_PLANT_DAY), ("WHEAT", 29)):
            if day > last_day:
                continue
            need = _empty_role_tiles(farm, self.roles, crop)
            have = int(seeds.get(crop, 0) or 0)
            want = max(0, need - have)
            # Wheat is feed infrastructure, so it draws on all cash; melon and
            # carrot are discretionary and must not raid the engine's claim.
...
class SpatialScheduler:
    """Greedy nearest-unit task assignment with one-unit-per-tile reservation.

    Deliberately NOT multi-agent pathfinding: units may share tiles (the env
    allows it), so there are no collisions to resolve and the only cost of the
    greedy choice is a little travel. Upgrade path, if the arena ever shows
    >5% idle unit-turns with headroom to spare: replace `_assign` with a
    Hungarian/auction assignment over (unit, task) travel costs.
    """
        ...
        for y in range(board):
            for x in range(board):
                ...
                if tile is None:
                    ...
                    elif role in ("MELON", "WHEAT"):
                        crop = role
                        last = MELON_LAST_PLANT_DAY if crop == "MELON" else 29
                        # Only plant if it can still reach harvest before the end.
                        if day <= last and seed_budget.get(crop, 0) > 0:
                            if day + CROPS[crop]["max_yield_day"] <= 29:
                                seed_budget[crop] -= 1
    ...
    def _animal_tasks(self, tile, x, y, day, days_left, market):
        ...
        # CARE banks +1 yield paid out on the next production tick — one action
        # for roughly one extra product, which more than doubles goose output.
        if FLAGS["ANIMAL_CARE"] and not cared and worth_keeping:
            if units < a["max_held"]:
                out.append((PRIO_CARE, x, y, ["CARE"], None))

        # Deliberately NOT collecting fertilizer. FERTILIZER cannot be sold (only
        # bought), melon already reaches its max_yield of 6 on watering alone, and
        # on wheat the whole play is +2 units for two actions (COLLECT then
        # FERTILIZE) — well under the ~$40/action a CARE on a goose returns.
        # Measured cost of collecting it anyway: 36 shed slots of a 100 cap
        # occupied by an unsellable item, plus the wasted actions.
        return out

    def assign(self, tasks, units, farm, private, board, n_animals, roles, log):
        """Greedy: walk tasks by priority, give each to its nearest capable free
        unit. One task per unit, one unit per tile."""
        actions = {}
        used_units = set()
        used_tiles = set()
        ...
        for i, pos in idle:
            actions[i] = self._logistics(i, pos, farm, private, board, n_animals)
    ...
    def _ferry_animals(
        ...
        for _x, _y, animal in open_vac:
            if int(shed.get(animal, 0) or 0) > sum(1 for a in fetchable if a == animal):
    ...
    def _provision_feed(self, actions, used_units, units, farm, private, board, log):
    ...
    def _logistics(self, idx, pos, farm, private, board, n_animals):
        ...
        if adjacent:
            ...
            if n_animals > 0 and int(inv.get("WHEAT", 0) or 0) < WHEAT_CARRY_PER_UNIT:
                if int(shed.get("WHEAT", 0) or 0) > 0:
                    return ["PICKUP", "WHEAT", WHEAT_CARRY_PER_UNIT]
        ...
        if carrying > 0 or need_feed or int(shed.get("GOOSE", 0) or 0) > 0:
            target = _nearest_shed_access(pos, farm, board)
            if target:
                step_op = _step_toward(pos, target, farm, board)
                if step_op:
