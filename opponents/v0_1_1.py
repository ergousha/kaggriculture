...
FLAGS = {
    ...
    # OFF on measurement, v0.1.1. The mechanics are real -- fertilizer doubles the
    # yield step for both crop families, taking wheat from 4 units to its cap of 6
    # and strawberry from 4 over its life to 8 -- and the whole 08-10 field runs ~73
    # FERTILIZE ops while we run 0. It still loses: 36.7% at PRIO_FERTILIZE=660 and
    # 33.3% at 300 or 1, against 56.7% with the task absent.
    #
    # The giveaway is that 300 and 1 produce byte-identical results (delta +0 over 30
    # paired seeds), i.e. at those priorities the task is never the reason a unit was
    # picked -- yet the agent still drops 23 points. So the cost is not the fertilizer
    # spent or the priority chosen, it is that ANY extra task consumes the leftover
    # units that `_logistics` needs. That is the same mechanism already recorded for
    # IDLE_PREPOSITION and for the Hungarian assignment: greedy's idle units cluster
    # near the shed, and every sack of feed and every animal enters the farm through
    # a shed PICKUP. Fertilizing is worth less than ferrying.
...
# Wheat. v0.1.1 promotes it from feed infrastructure to the primary cash crop:
# shortest cycle in the game and the only glut-proof curve. WHEAT_TILE_TARGET is an
# absolute floor and WHEAT_LAND_FRACTION claims a share of whatever land is left
# after pastures, strawberry and melon; WHEAT_TILES_PER_ANIMAL is retained purely as
# a feed-self-sufficiency floor.
WHEAT_TILE_TARGET = 14
WHEAT_LAND_FRACTION = 0.70
...
# v0.1.1. A yield multiplier, so it sits just under WATER_BONUS -- the fertilizer
# bonus only pays on a day the tile is actually watered. Deliberately BELOW the
# animal chores: raising PRIO_PLANT above CARE measured 2W-28L because the animal
# lines are 78% of our revenue, and there is no reason to think this is different.
PRIO_FERTILIZE = 660
...
class StrategicPlanner:
    ...
    def plan_roles(self, farm, day, days_left, market, cash, opp=None):
        ...
        if FLAGS["EGG_ENGINE"]:
            ...
            coop_n = 0
        rest = max(0, rest - coop_n)

        # Wheat is a cash crop, not just feed infrastructure. v0.1.1.
        #
        # This was `max(3, ceil((n_sheep + n_cows + coop_n) * WHEAT_TILES_PER_ANIMAL))`,
        # which pinned it near 8 tiles and never scaled with land. Across 28 live
        # v0.1.0 episodes we finished with ~0 wheat tiles and sold 29.7 wheat while
        # the opponents we actually faced sold 362.3 -- a 12.19x gap and the single
        # largest in the fingerprint. Wheat is the shortest capital cycle in the game
        # (seed $10, first_yield_day 2, max_yield 6, `ongoing: False` so the tile
        # dies on harvest and is immediately replantable) and its above-target curve
        # is `log` with amp 0.83, the flattest of any product: dumping 360 units from
        # I0 still prices at $20, where wool, milk, strawberry and melon have all hit
        # the $1 floor. It is the only glut-proof product in the game and we were
        # treating it as animal feed. The animal-derived figure is kept as a floor so
        # feed self-sufficiency never regresses.
        wheat_min = math.ceil((n_sheep + n_cows + coop_n) * WHEAT_TILES_PER_ANIMAL)
        wheat_n = min(rest, max(3, wheat_min, WHEAT_TILE_TARGET, int(rest * WHEAT_LAND_FRACTION)))
        ...
        # Tail: wheat, always. v0.1.1 -- this was STRAWBERRY while days_left >= 10.
        # Strawberry needs 10 days to its first yield and 16 to its last, so a tile
        # handed to it after ~day 13 returns little and after ~day 19 nothing, while
        # wheat yields from day 2 and can turn over five times in the same window.
        # The explicit STRAWBERRY_TILE_TARGET above already claims as much land as
        # strawberry can profitably use; the leftovers should go to the crop with the
        # shortest cycle, not the longest.
    ...
    def market_orders(self, obs, farm, private, market, opp, day, days_left, hour, log):
        ...
        #
        #    Emitting the SELLs before this so the turn's own revenue could fund the
        #    purchase was tried and measured EXACTLY neutral -- identical cash on every
        #    seed against ladder tiers 4, 5 and 6. The gate above already requires
        #    `discretionary - 500 >= price + LAND_CASH_BUFFER`, i.e. ~$2.5k of surplus,
        #    so cash sequencing never binds, and our order count sits under the
        #    10-per-turn cap. Reverted rather than kept: a zero-delta change is surface
        #    without gain, the same call made for PREDATORY_TIMING.
...
class SpatialScheduler:
    ...
    def _plant_tasks(self, tile, x, y, day, days_left):
        ...
        # Fertilize: doubles the yield step, on both crop families. v0.1.1.
        #
        # For a non-ongoing crop the whole yield comes from the WATER op, and
        # `_apply_unit_action` adds `2 if fertilized_until_day >= day else 1`, so
        # wheat's unfertilized ceiling is its planting unit plus one per window day
        # (1 + 3 = 4 of a max_yield 6) and fertilising the window reaches the cap.
        # For an ongoing crop `_daily_refresh_plants` adds `2 if fertilized else 1`
        # on each production day, so strawberry goes from 4 units over its life to
        # 8. `FERTILIZE` sets `fertilized_until_day = day + 2`, i.e. active for three
        # days inclusive, which covers two of strawberry's interval-2 production
        # days per application.
        #
        # `req="FERTILIZER"` restricts the task to a unit already carrying some --
        # FERTILIZE consumes from the acting unit's inventory, exactly like FEED --
        # and `COLLECT_FERTILIZER` deposits into that inventory, so the natural
        # chain is collect at a pasture then fertilize a nearby crop. Supply is not
        # scarce: `fertilizer_available` resets on every animal tile every day.
        if (
            FLAGS["FERTILIZE_CROPS"]
            and units < cd["max_yield"]
            and int(tile.get("fertilized_until_day", -1) or -1) < day
        ):
            window_start = (cd["max_yield_day"] + 1) // 2
            if cd["ongoing"]:
                # Only worth it if a production day falls inside the 3-day window.
                since = age - cd["first_yield_day"]
                interval = max(1, cd["interval"])
                useful = since >= -2 and any(
                    (since + k) >= 0 and (since + k) % interval == 0 for k in range(3)
                )
            else:
                useful = window_start <= age <= cd["max_yield_day"]
            if useful and FLAGS["FERTILIZE_CROPS"]:
                out.append((PRIO_FERTILIZE, x, y, ["FERTILIZE"], "FERTILIZER"))
        return out
