...
op_cols = [
    c
    for c in df.columns
    if c.startswith("op_") and c not in ("ops_productive", "ops_logistics", "ops_total")
]
...
for _, row in top_5.iterrows():
    ...
    if total_std < 5:
        ...
        print(
            "  Route Pattern: SEMI-STATIC (Likely a recorded route with slight runtime variations)"
        )
    else:
